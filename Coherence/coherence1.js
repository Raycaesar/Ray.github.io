...
function cutTail(anlist) {
    
    anlist.forEach(ele => {
        ele.pop();  // Removes the last item from the chain
        
    });
    
    return anlist;  // Optional: return the modified chains if needed
}
...
function checkCoherence() {
    ...
    let chainsNoTail = followingChain(); 

    cutTail(chainsNoTail);
...
return new Set([...acc].filter(x => set.has(x)));
    ...
    function isIntersectionNotEmpty(agent) {
        ...
        const section = getIntersection(sets);
    ...
    let followerCoherence = chainsNoTail.every(chain => chain.every(isIntersectionNotEmpty));
    ...
    if (allAgents.every(agent => !isDenotationEmpty(agent))) {
            weakCoherence = true;
        } 
...
function getDenotationResult2(agent) {
    if (agentBeliefs[agent] && typeof agentBeliefs[agent].denotation === 'string') {
        const denotationCore = agentBeliefs[agent].denotation.slice(1, -1);
        console.log(`agentBeliefs_${agent}.denotation`, agentBeliefs[agent].denotation);
